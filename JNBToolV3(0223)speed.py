...
def adminMode():
	...
	while True:
		pWinput = au.password(text = "Bạn cần nhập password để sử dụng chức năng này:", title = "Login")

		if pWinput == pW:
			cf = au.alert(text = "Login success, click OK to run active!")
			print("Login success")
			if cf == 'OK':
				return True
			else:
...
def removeMdb():
	# Lấy thông tin dự án tuwf file reg:

	regFile = open('C:\HDFormReg.dat')
	projCode = ''
	for line in regFile:
		pjl = line.split('|')
		if pjl[0] == "PROJECT_CODE":
			projCode = pjl[1][0:-1]
			break
		else:
			pass
	...
	dirProject = str('C:\HD-FORM2015Data\PROJECT\\'+ projCode + '\\')
	mdbList = []
	...
	return dirProject, projCode


def autoRun(fileNameTxt, info = ("None", "None", "중A", "None","None")):
	...
	for block in ls:
		name = "%s동%s_계단_%s(%s%s)" %(block[0], block[1],info[2],info[3],info[4])
		pyperclip.copy(name)
...
- ..... !!!</b></html> """)
		...
		self.pathTxtLine.setPlaceholderText("Nhấn 'Load Txt File...' để lựa chọn file Txt chứa danh sách tòa và khu vực.")
		...
		self.autoRunButton.setToolTip('Autorun to clone file mdb.')
		self.autoRunButton.setEnabled(True)
		...
		self.autoRunButton.clicked.connect(self.runClick)
	...
	def importTxtClick(self):
		...
		if fileName == "":
			...
			report = '''
						...
						\n- Xóa hết các file "mdb" không phải mặc định của Project đang làm ra khỏi ở ổ C tại đường dẫn "C:\HD-FORM2015Data\PROJECT\~CODE...".
						\n- Trong quá trình chạy, tránh tác động vào chuột và bàn phím vì có thể gây ra lỗi.
						\n- Nhấn nút "Run..." nếu đã sẵn sàng.
						''' % fileName.upper()
	...
	def runClick(self):
		...
		if self.pathTxtLine.text() == "":
			report = " Bạn chưa chọn file muốn chỉnh sửa, hãy nhấn 'Import Txt File....' để chọn."
		else:
			# os.startfile(self.pathTxtLine.text())
			nameFile = self.pathTxtLine.text()
			try:
				if Error:
					report = "Đang tự tạo file MDB tự động, chờ ít phút... Để tránh gặp lỗi ngoài mong muốn"
					oldDir = removeMdb()[0]
					prCode = removeMdb()[1]
					autoRun(nameFile, self.info)
					...
					if newDir == '':
						...
						pass
					print(newDir)
					moveMdb(oldDir, prCode, newDir)
					report = "File mdb được lưu tại %s" % newDir
					os.startfile(newDir)
				else:
					report = ""
			except:
				print("loi o day")
				report = "Xuất hiện lỗi, hoặc người dùng đã dừng thao tác tạo MDB tự động."
	...
	def contMesBoxReply(self):
		if self.toaLine.text() == "" or self.kvLine.text() == "" or self.DesLine.text() == "":
			self.contMesBoxRep = QMessageBox.question(self, 'PyQt5 message', "Tên tòa, khu vực, designer chưa đầy đủ, bạn có muốn tiếp tục?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
